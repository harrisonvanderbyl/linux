...
static int qcom_smmu_cfg_probe(struct arm_smmu_device *smmu)
{
	 ...
	 * 128 stream matching groups. The additional registers have the same
	 * behavior as the architected registers in the hardware. However, on
	 * some firmware versions, the hypervisor does not correctly trap and
	 * emulate accesses to the additional registers, resulting in
	 * unexpected behavior.
	 *
	...
	if (FIELD_GET(ARM_SMMU_S2CR_TYPE, reg) != S2CR_TYPE_BYPASS) {
		...
		if (smmu->num_mapping_groups > 128) {
			dev_notice(smmu->dev, "\t" FW_BUG "Limiting the stream matching groups to 128\n");
